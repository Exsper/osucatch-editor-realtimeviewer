...
- **The viewer gets stuck at `Try fetch editor` with no further log output**: this was a memory-scan compatibility issue under Wine in older builds; the latest release fixes it (chunked reads plus a watchdog for regions where `ReadProcessMemory` blocks). If the new log shows `scan aborted ... stalled at region ...`, that region is backed off (retried later with a growing delay) and the next attempt resumes from where it stopped — that is expected, and the picture keeps updating while the re-scan runs in the background.
